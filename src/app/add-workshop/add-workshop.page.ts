...
export class AddWorkshopPage implements OnInit {
  ...
  async addwsfn(){
    ...
    if(! this.location_fetched){
      ...
      console.log(this.addcwForm.value)
      this.addcwForm.value.dealer_name = this.selected_dealer.pk

      await loading.present();
      this._apiservice.postForm("api/sales/add_workshop",this.addcwForm.value)
        .subscribe(data => { 
              loading.dismiss(); 
              this.presentAlert("Sucess!!!","Workshop Added Successfully","");
              this.location.back();
